...
An example of an observation would be for the user id `r {instacart %>% select(user_id) %>% head(1)}`, a product they ordered was `r {instacart %>% select(product_name) %>% head(1)}`. This product comes from the `r {instacart %>% select(department) %>% head(1)}`.
